from Utilities_Forecasting_Dlinear_syndata import *
import optuna
...
if __name__ == "__main__":
    ...
    train_synthetic, val_synthetic = train_test_split(data=synthetic_df, test_size=0.2)

    syn_load_train = TimeSeries.from_dataframe(df=train_synthetic, value_cols=["TOTALDEMAND"])
    syn_load_val = TimeSeries.from_dataframe(df=val_synthetic, value_cols=["TOTALDEMAND"])
    syn_future_covs_train = TimeSeries.from_dataframe(df=train_synthetic, value_cols=["Weekday", "Day_of_year", "Hour_of_day",
                                                                            "public_holidays"])
    syn_future_covs_val = TimeSeries.from_dataframe(df=val_synthetic, value_cols=["Weekday", "Day_of_year", "Hour_of_day",
                                                                        "public_holidays"])
    ...
    def objective(trial):
        ...
        dlinear_model.fit(series=syn_load_train, future_covariates=syn_future_covs_train)

        trial_number = len(study.trials)
        model_name = f"dlinear_model_trial{trial_number}"

        backtest_naive, backtest_metrics, load_series = evaluation(syn_load_val, dlinear_model, syn_future_covs_val, measurements_per_day)
